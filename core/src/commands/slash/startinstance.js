...
module.exports = {
  ...
  run: async (client, interaction) => {    
    ...
    let token = await db.getInstance(interaction.options.get("id").value)
         ...
         process.createProcess(interaction.options.get("id").value, `node`, [`./index.js`, `${token[0]['token']}`, `${interaction.user.id}`], client, `./src/instances/bots/${interaction.options.get("id").value}/`)
