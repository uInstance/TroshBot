...
module.exports = {
    ...
    run: async (client, message, args, prefix, color) => {
            ...
            if(process.argv[3] === message.author.id || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || perm) {    
                ...
                if (!process.argv[3] === message.author.id || db.get(`ownermd_${client.user.id}_${message.author.id}`) === null  && role.permissions.has("KICK_MEMBERS") || role.permissions.has("BAN_MEMBERS") ||  role.permissions.has("MANAGE_WEBHOOKS") ||role.permissions.has("ADMINISTRATOR") || role.permissions.has("MANAGE_CHANNELS") || role.permissions.has("MANAGE_GUILD") || role.permissions.has("MENTION_EVERYONE") || role.permissions.has("MANAGE_ROLES")) {
