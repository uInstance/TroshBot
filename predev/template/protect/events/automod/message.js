...
module.exports = async (client, message) => {
    ...
    if (db.get(`linkwl_${guild.id}`) === null) perm = client.user.id === message.author.id || guild.owner.id === message.author.id || process.argv[3] === message.author.id || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || db.get(`wlmd_${guild.id}_${message.author.id}`) === true
    if (db.get(`linkwl_${guild.id}`) === true) perm = client.user.id === message.author.id || guild.owner.id === message.author.id || process.argv[3] === message.author.id || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true
